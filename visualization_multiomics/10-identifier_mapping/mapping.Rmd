...
```
## Process gene data
```{r gene_data, warning=FALSE, message=FALSE}
#read data
...
data <- data [,-2]
colnames(data) <- c("Identifier","log2FC","pvalue","data.type")
```
...
colnames(merged.data) <- c("Identifier","log2FC","pvalue","data.type")
```

# Combine transcriptomics and metabolimics data
```{r combine, warning=FALSE, message=FALSE}

combined <- rbind(data,merged.data)
write.table(combined, "output/combinedData",sep="\t",row.names = FALSE, quote = FALSE)
```
