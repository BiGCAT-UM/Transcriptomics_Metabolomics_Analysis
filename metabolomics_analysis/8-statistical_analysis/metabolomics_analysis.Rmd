...
remove(mSet_transformed, transformation, columns_disorders, columnNumber, end_Disorders, foldchange_disorder, p_values_disorder, control_IBD, disease, ttest_mSet, mSet_FINAL)
```
...
##Move next section to separate RMD file (after PW analysis)

## Add labels to the IDs with BridgeDb
if (!requireNamespace("BiocManager", quietly = TRUE)) install.packages("BiocManager")
if(!"rJava" %in% installed.packages()){install.packages("rJava")} ## See https://www.r-bloggers.com/2018/02/installing-rjava-on-ubuntu/ if you have issues with this package on Ubuntu.
if(!"BridgeDbR" %in% installed.packages()) BiocManager::install("BridgeDbR")
library(BridgeDbR)

##Download the Metabolite mapping file: (only if file is not present yet TODO)
if(file.exists("data/metabolites.bridge")){print("Metabilte Mapping file already downloaded")}else{
